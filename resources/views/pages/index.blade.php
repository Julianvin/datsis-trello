...
<div class="content flex-grow p-4 md:p-8">
    ...
    <!-- Form Pencarian -->
    <div class="flex flex-col md:flex-row justify-between items-center mb-6" data-aos="fade-down" data-aos-delay="100">
        <a href="{{ route('siswa.data') }}"
            class="inline-block bg-gray-600 text-white px-6 py-2 rounded-full text-sm font-semibold hover:bg-gray-700 transition duration-300 ease-in-out mb-4 md:mb-0">
            <i class="fa-solid fa-arrow-left"></i> CRUD Data Siswa
        </a>
        <form action="{{ route('landing_page') }}" method="GET" class="flex items-center w-full md:w-auto"
            data-aos="fade-left" data-aos-delay="200">
            <input type="text" name="search" placeholder="Cari nama, NIS, rayon, atau rombel..."
                class="w-full md:w-0 md:flex-1 transition-all duration-300 ease-in-out p-3 border border-gray-300 rounded-l-lg focus:outline-none focus:ring-2 focus:ring-blue-600 shadow-sm"
                value="{{ request('search') }}" onfocus="this.classList.add('md:w-96');" onblur="if (this.value == '') { this.classList.remove('md:w-96'); }">
            <button type="submit"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-6">
        ...
        <div class="bg-yellow-300 rounded-lg shadow-md p-4" data-aos="zoom-in-up" data-aos-delay="100">
            <p class="text-center font-bold text-xl">Belum ada data yang diinput, tolong input data terlebih dahulu!</p>
        </div>
        @endforelse
    </div>
